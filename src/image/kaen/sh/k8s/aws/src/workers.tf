...
data "template_file" "init_k3s_agent" {
  count    = var.workers
  template = file("${path.module}/init_k3s_agent.sh")
  ...
  vars = {    
    ...
    k3s_manager_private_ip    = aws_instance.managers[0].private_ip
    k3s_manager_protocol      = var.k3s_manager_protocol
    k3s_manager_port          = var.k3s_manager_port
...
data "cloudinit_config" "workers" {
  ...
  part {
    filename     = "init_k3s_agent.sh"
    content      = data.template_file.init_k3s_agent[count.index].rendered
    content_type = "text/x-shellscript"
  }
